...
(() => {
  ...
  const uploadPlaceholderTitle=document.getElementById('uploadPlaceholderTitle');
  const uploadPlaceholderText=document.getElementById('uploadPlaceholderText');
  const uploadPortalOpen=document.getElementById('uploadPortalOpen');
  if(uploadFrame&&uploadPlaceholder){
    const url=String(window.CNP_UPLOAD_PORTAL_URL||'').trim();
    if(url){
      if(uploadPortalOpen){uploadPortalOpen.href=url;uploadPortalOpen.hidden=false;}
      uploadFrame.addEventListener('load',()=>{
        uploadFrame.classList.add('active');
        uploadPlaceholder.hidden=true;
      },{once:true});
      uploadFrame.addEventListener('error',()=>{
        if(uploadPlaceholderTitle)uploadPlaceholderTitle.textContent='The embedded portal could not be loaded.';
        if(uploadPlaceholderText)uploadPlaceholderText.textContent='Use the button below to open the secure upload portal directly.';
      },{once:true});
      uploadFrame.src=url;
      window.setTimeout(()=>{
        if(!uploadFrame.classList.contains('active')){
          if(uploadPlaceholderTitle)uploadPlaceholderTitle.textContent='The portal is taking longer than expected to load.';
          if(uploadPlaceholderText)uploadPlaceholderText.textContent='Open it in a new tab or verify the Apps Script deployment URL.';
        }
      },7000);
    }else{
      if(uploadPlaceholderTitle)uploadPlaceholderTitle.textContent='Secure upload is not connected yet.';
      if(uploadPlaceholderText)uploadPlaceholderText.innerHTML='Deploy the included Google Apps Script web app, then paste its <code>/exec</code> URL into <code>assets/upload-config.js</code>.';
      if(uploadPortalOpen)uploadPortalOpen.hidden=true;
    }
  }
